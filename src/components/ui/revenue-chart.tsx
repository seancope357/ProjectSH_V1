...
export function RevenueChart({
  ...
  // Scales
  const xScale = (d: Date) => {
    const t = (d.getTime() - domain.minX.getTime()) / (domain.maxX.getTime() - domain.minX.getTime() || 1)
    return padding.left + t * plotW
  }
  const yScale = (v: number) => padding.top + plotH - ((v - domain.minY) / (domain.maxY - domain.minY || 1)) * plotH
  ...
  const buildPath = (pts: { date: Date; value: number }[]) => {
    ...
    return pts.map((p, i) => `${i === 0 ? 'M' : 'L'} ${xScale(p.date)} ${yScale(p.value)}`).join(' ')
  }
  ...
  const nearest = useMemo(() => {
    ...
    const distances = series.primary.points.map(p => Math.abs(xScale(p.date) - hoverX))
  ...
  return (
    <div ref={containerRef} className={`w-full ${className ?? ''}`}>
      <div className="text-sm text-gray-700 flex items-center justify-between mb-2">
        ...
        <div className={`text-xs ${trendDelta >= 0 ? 'text-green-600' : 'text-red-600'}`}>
      ...
      <svg viewBox={`0 0 ${width} ${vbHeight}`} className="w-full h-auto">
        ...
        {/* X axis */}
        <line x1={padding.left} y1={padding.top + plotH} x2={padding.left + plotW} y2={padding.top + plotH} stroke="#E5E7EB" />
        ...
        {series.comparison?.points?.length ? (
          <path d={buildPath(series.comparison.points)} stroke={comparisonColor} strokeWidth={1} fill="none" opacity={0.6} />
        ) : null}

        {/* Primary series */}
        {series.primary?.points?.length ? (
          <>
            <path d={buildPath(series.primary.points)} stroke={primaryColor} strokeWidth={1.5} fill="none" />
          ...
          <text x={padding.left + plotW / 2} y={padding.top + plotH / 2} textAnchor="middle" fontSize={12} fill="#9CA3AF">
            No data
          </text>
        )}
        ...
        {nearest ? (
          <g>
            <line x1={xScale(nearest.point.date)} y1={padding.top} x2={xScale(nearest.point.date)} y2={padding.top + plotH} stroke="#D1D5DB" strokeDasharray="4 2" />
            <circle cx={xScale(nearest.point.date)} cy={yScale(nearest.point.value)} r={3} fill={primaryColor} />
            <foreignObject x={xScale(nearest.point.date) - 60} y={yScale(nearest.point.value) - 50} width={120} height={38}>
              <div className="pointer-events-none select-none rounded bg-white shadow px-2 py-1 text-xs text-gray-800 border border-gray-200">
                <div className="font-medium">{formatCurrency(nearest.point.value, currency)}</div>
                <div className="text-gray-600">
                  {new Intl.DateTimeFormat('en-US', { month: 'short', day: 'numeric', year: '2-digit' }).format(nearest.point.date)}
                </div>
              </div>
            </foreignObject>
          </g>
