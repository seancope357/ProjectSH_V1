...
export function AuthButton() {
        ...
        {isOpen && (
          <>
            ...
            <div className="absolute top-full mt-2 w-full min-w-48 bg-white rounded-lg shadow-xl border border-gray-200 z-20 overflow-hidden">
              <Link
                href="/profile"
                className="block px-6 py-4 text-gray-700 hover:bg-blue-50 hover:text-blue-700 transition-colors font-medium"
                onClick={() => setIsOpen(false)}
              >
                Profile
              </Link>
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="bg-white text-blue-700 px-6 py-3 rounded-lg text-lg font-semibold hover:bg-blue-50 transition-all duration-300 flex items-center justify-center shadow-lg hover:shadow-xl transform hover:-translate-y-1 border-2 border-blue-700"
      >
        <User className="mr-2 h-5 w-5" />
        Sign In / Register
        <ChevronDown className="ml-2 h-4 w-4" />
